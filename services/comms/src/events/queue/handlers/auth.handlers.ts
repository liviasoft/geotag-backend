...
export const USER_SIGNED_UP_VIA_EMAIL = async (message: ServiceEvent<any>): Promise<ServiceResponse<any>> => {
  ...
  if (settings.REQUIRE_VERIFICATION_ON_SIGNUP) {
    ...
    try {
      ...
      console.log({ error });
      return statusTypes.get('InternalServerError')!({ error });
    }
...
export const USER_SIGNED_UP_VIA_PHONE = async (message: ServiceEvent<any>): Promise<ServiceResponse<any>> => {
  const settings = await getAppSettings();
  const upgs = new UserPostgresService({});
  console.log({ message });
  if (settings.REQUIRE_VERIFICATION_ON_SIGNUP) {
    ...
    // TODO: Get user verification data
    const cs = new CacheService();
    const verifData = (await cs.formatKey({ scopeToService: false }, 'OTP', 'PHONE', userId).get())
      .result! as TVerificationData;
    console.log({ verifData });
    // ? Shorten token URL?
    // TODO: Get signup SMS template
    const messagingService = new MessagingService(['SMS'], settings);
    try {
      await messagingService.prepareTemplate('USER_SIGNUP_VERIFICATION_CODE');
      console.log({ messagingService });
      // await messagingService.sendMessage()
    ...
    // TODO: Populate Template and formulate SMS
    // TODO: Send signup SMS
  }
  return statusTypes.get('OK')!({});
};
